...
namespace MovieApp.UI.Controllers
{
    public class ThetreController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> UpdateThere(int id)
        {
            string URL=_configuration["WebApiURL"] + "Thetre/fetchthetre?id="+id;
            using(HttpClient httpClient = new HttpClient())
            {
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateThere(ThetreModel thetreModel)
        {
            StringContent body = new StringContent(JsonConvert.SerializeObject(thetreModel), Encoding.UTF8, "application/json");
        ...
        [HttpPost]
        public async Task<IActionResult> DeleteThetre(int id)
        {
            string endpoint = _configuration["WebApiURL"] + "Thetre/deletethetre?id="+id;
            using(HttpClient client=new HttpClient())
            {
